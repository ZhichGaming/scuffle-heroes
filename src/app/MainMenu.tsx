...
export default function MainMenu({ gameInfo, handlePressStart }: { gameInfo: GameInfo | undefined, handlePressStart: () => void }) {
    return (
        <div className='bg-[url("/Regular.webp")] h-screen w-screen bg-cover flex flex-col'>
            ...
            <div className='flex justify-center items-center space-x-6 mb-10'>
                <button className={`bg-gray-700 w-80 h-20 rounded-md text-white border-black border shadow-md bg-cover bg-center`} style={{ backgroundImage: `linear-gradient(rgba(55, 65, 81, 0.5), rgba(55, 65, 81, 0.9)), url("/banners/${gameInfo?.map.bannerImageFileName || ''}")` }}>
                    <p className='text-lg font-bold'>{gameInfo?.gameMode || <Skeleton width={100} containerClassName="flex-1"/>}</p>
                    <p className='text-sm text-gray-300'>{gameInfo?.gameMode && gameModeDescriptions[gameInfo.gameMode] || <Skeleton width={300} containerClassName="flex-1"/>}</p>
                </button>
                <button className='bg-yellow-500 w-64 h-20 rounded-md font-bold text-white text-3xl border-black border shadow-md'>
                    <p className='font-outline-1'>PLAY</p>
                </button>
            </div>
        </div>
    );
